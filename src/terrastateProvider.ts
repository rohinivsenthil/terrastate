...
class Item extends vscode.TreeItem {
    ...
    constructor({ type, tfstateFile, resource }: { type: string, tfstateFile?: vscode.Uri, resource?: any  }) {
...
export class TerrastateProvider implements vscode.TreeDataProvider<Item> {
    ...
    constructor() {
        ...
        fsWatcher.onDidCreate(created => {
            ...
            this.rootItems.sort((a, b) => ((a.label || "") > (b.label || "")) ? 1 : -1);
        ...
        if (element?.tfstateFile) {
            try {
                ...
                const items = data.resources.map(
                    (resource: any) => new Item({ type: 'resource', resource: resource })
                );
